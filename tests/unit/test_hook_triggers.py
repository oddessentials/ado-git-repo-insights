...
class TestIsPrRecordParityTrigger:
    """is_pr_record_parity_trigger must cover exactly the two files that
    ``scripts/check_pr_record_schema_parity.py`` parses (Feature 310, QG-47).

    Scope is intentionally exact — the gate parses two files and the
    ...
    any file under ``src/ado_git_repo_insights/`` that is not ``types.py``).
    """
    ...
    def test_parity_gate_read_paths_are_triggers(self, path: str) -> None:
        """The two files the parity gate parses MUST each fire the dispatch."""
    ...
    def test_near_miss_paths_are_not_triggers(self, path: str) -> None:
        """Exact-match scope rejects everything outside the two read paths."""
