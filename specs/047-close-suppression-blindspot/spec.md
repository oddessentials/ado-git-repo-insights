...
- **Phase 1 (advisory (non-blocking))**: The expanded audit runs in all entry points but reports the new scopes as warnings, not failures. The existing 3 scopes remain blocking. This lets the team see the real count and track progress without halting development.
...
- What happens during Phase 1 if a developer adds a NEW suppression to a new-scope file? The advisory (non-blocking) audit warns but does not fail. This is acceptable because Phase 1 is a visibility phase — the team can see the increase. Phase 2 enforcement blocks it.
...
- **FR-018**: The audit tool MUST include a file-coverage check that asserts every `.py` file belongs to exactly one scope and fails with a list of uncovered paths if any file is unscoped. The full-tree check MUST run in preflight and CI. Pre-commit MUST run a staged-subset check (see FR-026 for precise tier contracts).
- **FR-019**: The expanded audit MUST support two-phase gating: Phase 1 runs the new scopes in advisory (non-blocking) (warning) mode while existing scopes remain blocking; Phase 2 switches all scopes to blocking once the verified census reaches zero for the new scopes. The phase transition MUST be a baseline update (configuration change), not a code change.
...
- **SC-014**: Two-phase gating is operational: new scopes report warnings (advisory (non-blocking)) in Phase 1 and switch to blocking in Phase 2 via a baseline update — no code change required for the transition.
...
- Two-phase gating assumes that development on other features continues during the cleanup period. Phase 1 (advisory (non-blocking)) prevents the expanded audit from halting unrelated work while cleanup proceeds.
