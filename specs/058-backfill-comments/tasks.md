...
- [ ] **T001** Add `FetchOutcome` frozen dataclass + required imports in `src/ado_git_repo_insights/cli.py`
  ...
  - Add module imports as needed: `Literal` from `typing`, `Mapping` from `collections.abc`, `dataclass` from `dataclasses`. Confirm `AdoThread` is already importable from `.extractor.ado_client` (Pass 3 code-validation re-verifies); otherwise declare `AdoThread = Mapping[str, object]` as a local type alias adjacent to the dataclass.
...
- [ ] **T002** Extract `_fetch_and_upsert_threads_for_pr` helper from current per-PR inline body
...
- [ ] **T003** Refactor `_extract_comments` to call `_fetch_and_upsert_threads_for_pr`; preserve extract's caller-side pre-iteration snapshot + post-fix 3-case stamp (with Case 2 sub-decision) + end-of-loop commit bit-for-bit
