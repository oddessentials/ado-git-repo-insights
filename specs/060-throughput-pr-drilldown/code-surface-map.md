...
| FR-013 | Public publish MUST NOT contain `prs` | `scripts/build-demo-dataset.py:1044` (`promote_data`) is the single authoritative write boundary to `docs/data/`. `scripts/generate-demo-data.py:105` has `DEFAULT_OUTPUT_DIR = docs/data/` but is orchestrated via `run_generator(script, ARTIFACT_DATA_DIR)` (line 1095), so the default is only reachable via standalone developer invocation | New helper `scripts/strip_pr_arrays.py` invoked as the FIRST step inside `promote_data` when destination is `DOCS_DATA_DIR`. Standalone-bypass closure: change `generate-demo-data.py` DEFAULT_OUTPUT_DIR to a scratch path + add early-exit guard rejecting `--output-root == DOCS_DATA_DIR` |
...
| FR-023 | Demo-publish enforcement gate | Single authoritative write boundary: `promote_data` at `scripts/build-demo-dataset.py:1044`. Orchestrated flow: `build-demo-dataset.py` → `run_generator(script, ARTIFACT_DATA_DIR)` at line 1095 → `promote_data(ARTIFACT_DATA_DIR, DOCS_DATA_DIR)` at line 1120. `generate-demo-data.py:105` `DEFAULT_OUTPUT_DIR = docs/data/` is a standalone-developer bypass path (NOT exercised in orchestration) | New helper `scripts/strip_pr_arrays.py` exporting `strip_pr_arrays_from_rollups`. Gate invocation: INSIDE `promote_data` as its FIRST step when destination equals `DOCS_DATA_DIR`. Bypass closure: change `generate-demo-data.py` `DEFAULT_OUTPUT_DIR` to a scratch location + early-exit guard rejecting `--output-root == DOCS_DATA_DIR`. Static invariant test: grep `scripts/` + `.github/workflows/` — fail if any write to `docs/data/` does NOT go through `promote_data` |
...
1. **FR-023 entry-point wording**: evolved across two revisions. Pass 4 initially enumerated multiple write-paths (`build-demo-dataset.py`, `generate-demo-data.py`, three CI workflows). A subsequent source-verification re-alignment (Codex catch #4) collapsed this to a single authoritative write boundary (`promote_data` at `build-demo-dataset.py:1044`) with the gate invoked inside that helper, plus a separate bypass-closure for the `generate-demo-data.py` standalone path. The currently-authoritative text is the `spec.md` FR-023 body; this historical note is retained only to trace the evolution.
...
- **FR-023 re-aligned to single authoritative write boundary** (superseded the earlier Pass-4 enumeration of multiple write-paths after source-verification revealed `promote_data` at `build-demo-dataset.py:1044` is the only production write boundary): strip gate lives INSIDE `promote_data` as its first step when destination equals `DOCS_DATA_DIR`; single authoritative helper `strip_pr_arrays_from_rollups`; strip-and-re-verify (not strip-or-verify); bypass-closure for `generate-demo-data.py` standalone path via `DEFAULT_OUTPUT_DIR` change + early-exit guard; static invariant test forbids any write to `docs/data/` outside `promote_data`.
...
| Demo-strip gate runs before every publish | FR-023 re-aligned after source verification: single production write boundary is `promote_data` at `build-demo-dataset.py:1044`; strip gate lives inside `promote_data` as its first step when destination equals `DOCS_DATA_DIR`. Standalone-bypass closure for `generate-demo-data.py` handled by `DEFAULT_OUTPUT_DIR` change + early-exit guard (not a duplicate gate). Static invariant test forbids any direct write to `docs/data/` outside `promote_data`. |
