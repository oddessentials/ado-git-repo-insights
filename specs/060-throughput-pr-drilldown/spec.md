...
- **FR-003**: The contract surface for truncation state MUST expose exactly four accessors, locked now as the single source of truth for every consumer (panel UI, tests, future downstream):
   ...
   Consumers MUST NOT compute `_prs_truncated` or `_prs_cap` independently; both are read from the payload. The truncation indicator (FR-017) MUST use `rendered_count` and `actual_filtered_count` together when they differ, so users see both the visible rendering and the underlying filtered population.
...
- **FR-021**: Under any supported-filter state (including combined author+repository), the filtered PR array and the filtered `pr_count` MUST be produced by a single invocation of `applyFiltersToRollups` (or its direct authoritative replacement). The function's return shape is extended so that both filtered outputs are produced in the same operation against the same input rollup object; a parallel filter function, a cached intermediate, or a second invocation of filter logic for the PR list is FORBIDDEN. When the rollup's `prs` array is the full week population (i.e., `_prs_truncated=false`), the set of PR records rendered in the panel MUST be element-wise identical to the set of PRs that the aggregate filter code path attributes to the week. When the rollup's `prs` array is a top-cap truncated subset (`_prs_truncated=true`), the rendered set MUST be the filter applied to that truncated array — i.e., a subset of the full aggregate-attribution set — and the truncation indicator (FR-008, FR-017) MUST surface both counts so the user can see the rendered set is incomplete. Tests MUST cover both cases.
...
- **FR-023**: Every code path that writes a weekly-rollup artifact into the `docs/data/` public-surface tree MUST execute the same single authoritative enforcement gate as the last step before the artifact lands in `docs/data/`. The complete set of current write-paths is enumerated here (any new path introduced in the future MUST invoke the same gate or the build fails):
   1. `scripts/build-demo-dataset.py` — the canonical demo-dataset promotion (`DOCS_DATA_DIR` line 45; `atomic_replace_docs_data` at line 1045+).
   2. `scripts/generate-demo-data.py` — direct-to-`docs/data/` generator (`DEFAULT_OUTPUT_DIR` at line 105).
   3. Every CI workflow step in `.github/workflows/{ci,demo,release}.yml` that invokes either of the above.

   The gate MUST fail the write if any rollup artifact written under `docs/data/aggregates/weekly_rollups/` contains any of: a `prs` field, a `_prs_truncated` marker, a `_prs_cap` marker, or any PR-identifying field at PR granularity (PR id, PR title, PR author_id, PR repository_id attached to a PR record, or any derived URL). The gate MUST be a single authoritative helper (one function in one module) invoked by name from every write-path — copy-pasted inline checks are FORBIDDEN. The gate MUST strip-and-re-verify (not strip-or-verify): it strips PR-level fields, then re-scans the stripped output and fails the build if any residue remains. A synthetic test case in which an unstripped rollup is introduced MUST cause the gate to fail hard and block the write. Publishing to `docs/data/` with any such residue MUST be impossible to complete silently.
...
- **SC-011**: In every combined-filter (author+repository) fixture case, the set of PR records rendered in the panel equals element-wise the set of PR records that the aggregate filter code path attributes to the same week. Divergence in any case MUST fail the test.
