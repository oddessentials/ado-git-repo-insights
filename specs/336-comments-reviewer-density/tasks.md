...
- [ ] T007 [Phase 2.1] Add demo synthetic-stream coherence guard at `tests/unit/test_demo_synthetic_pr_comments.py`. Cover CL-14 step 3 / A-12 / ADR R005: (a) for every PR P in a small synthetic fixture, `len(synthetic_pr_threads for P) == P.thread_count`; (b) `len(synthetic_pr_threads for P with status='active') == P.active_thread_count`; (c) `len(synthetic_pr_comments for P) == P.comment_count`; (d) every emitted thread has ≥1 comment (no orphan threads); (e) every commenter `author_id` ≠ corresponding PR's `author_id`; (f) ghost-commenter inclusion when `ghost_pool` is non-empty (CL-14 step 4); (g) every emitted commenter `author_id` matches UUID format; (h) empty-week scenarios (empty PR list / all-empty PRs / all-partial-sentinel PRs) MUST return `([], [])` without raising — added post-T015 Codex stop-time review of commit 0705471e (CL-14 step 4 ghost-commenter inclusion is a DATASET-level guarantee, NOT a per-week one). Test MUST currently FAIL (synthetic streams not implemented yet). Bump `.test-floor-contract.json` Python floor by 7 in same commit (QG-43; widened from +6 to +7 by case (h) regression test). **Per ADR R005 / kickoff lesson "demo key-shape verification — do this FIRST", T007 MUST land before T008+ tests proceed.**
...
- **Cumulative test additions**: Phase 2 +30 Python +9 Extension; Phase 3 +12 Extension; Phase 4 +4 Extension; Phase 5 +4 Extension; Phase 6 +3 Extension; Phase 7 +3 Extension. **Cumulative target**: +30 Python + +35 Extension across the PR. Verified at task time via `scripts/check_ratchet_bump.py` (T033). Notes:
  ...
  - The original T007 plan listed 6 cases; case (h) empty-week regression added during T015 implementation post-Codex stop-time review of commit 0705471e (CL-14 step 4 ghost-commenter inclusion is a DATASET-level guarantee, NOT per-week — synthesizer must return `([], [])` on weeks with no comments rather than raising).
