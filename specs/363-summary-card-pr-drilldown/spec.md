...
- **SC-003**: For a rollup window where every contributing rollup honors the PR-list cap and `_prs_cap` envelope, the period-scoped PR list never renders more rows than `max(per-week _prs_cap)` would permit; when at least one contributing rollup signals truncation OR the collected count is strictly less than the period-level pr_count sum, the truncation cue is rendered.
...
- The existing `_prs_cap` semantics on a per-rollup basis are stable across rollup windows; the period-scoped envelope is `max(per-week)` — this mirrors reviewer-drilldown's already-shipped contract (`reviewer-drilldown.ts:316`) and is therefore an inherited invariant, not an assumption.
