...
**Scale/Scope**: ≤500 PR records per week × ≤52 weeks ≈ ≤26,000 PRs in the worst-case period union. The renderer caps the visible list per `actualFilteredCount` / `capValue` envelope and emits the truncation cue when the union exceeds the cap. No new cap mechanism introduced.
