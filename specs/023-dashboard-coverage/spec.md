...
- **FR-007**: Coverage thresholds MUST enforce elevated line coverage on Critical Path Set files: 70% for `dashboard.ts`, 40% for `artifact-client.ts`, 95% for `security.ts` (Jest enforces per-file thresholds; critical functions `renderPredictionsForState`, `renderInsightsForState`, `getSourceConfig`, `loadDataset`, `resolveConfiguration`, `_authenticatedFetch` are covered by these file thresholds)
...
- **Critical Path Set**: High-risk files with elevated coverage thresholds—dashboard.ts (70%), artifact-client.ts (40%), security.ts (95%)—containing critical functions: `renderPredictionsForState`, `renderInsightsForState`, `getSourceConfig`, `loadDataset`, `resolveConfiguration`, `_authenticatedFetch`

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: Critical Path Set files achieve target coverage: dashboard.ts ≥70%, artifact-client.ts ≥40%, security.ts ≥95%
