    ...
    try {
        ...
        if (savedProjectId) {
            if (projectDropdownAvailable) {
                const dropdown = document.getElementById('project-select');
                if (dropdown)
                    dropdown.value = savedProjectId;
            }
            else {
                const textInput = document.getElementById('project-id');
                if (textInput)
                    textInput.value = savedProjectId;
            }
        }
        // Set pipeline ID
    ...
    return new Promise((resolve) => {
        VSS.require(['TFS/Build/RestClient'], (BuildRestClient) => {
            try {
                ...
                client.getDefinitions(projectId, null, null, null, 2, // queryOrder: definitionNameAscending
                null, null, null, [pipelineId]).then((definitions) => {
                    ...
                    client.getBuilds(projectId, [pipelineId], null, null, null, null, null, null, 2, 6, null, null, 1).then((builds) => {
            ...
            try {
                ...
                client.getDefinitions(projectId, null, null, null, 2, 50).then(async (definitions) => {
                    for (const def of definitions) {
                        ...
                        try {
                            const builds = await client.getBuilds(projectId, [def.id], null, null, null, null, null, null, 2, 6, null, null, 1);
                            if (!builds || builds.length === 0)
                                continue;
    ...
    try {
        ...
        for (const match of matches) {
            document.getElementById(`select-pipeline-${match.id}`)?.addEventListener('click', () => {
                const pipelineInput = document.getElementById('pipeline-id');
                if (pipelineInput)
                    pipelineInput.value = match.id.toString();
...
window.selectDiscoveredPipeline = (pipelineId) => {
    const pipelineInput = document.getElementById('pipeline-id');
    if (pipelineInput)
        pipelineInput.value = pipelineId.toString();
