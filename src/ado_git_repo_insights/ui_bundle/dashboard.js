...
var PRInsightsDashboard = (() => {
  ...
  var DATASET_CANDIDATE_PATHS = [
    ...
    "aggregates",
    // Single nesting (common)
    "aggregates/aggregates",
    // Double nesting (ADO artifact download quirk)
    "dataset"
    // Alternative naming
  ];
  ...
  var currentDateRange = { start: null, end: null };
  var currentFilters = { repos: [], teams: [] };
  ...
  async function resolveFromPipelineId(pipelineId, projectId) {
    ...
    if (!builds || builds.length === 0) {
      const definitions = await buildClient.getDefinitions(projectId, void 0, void 0, void 0, 2, void 0, void 0, void 0, [pipelineId]);
  ...
  async function discoverInsightsPipelines(projectId) {
    const buildClient = await getBuildClient();
    const matches = [];
    const definitions = await buildClient.getDefinitions(projectId, void 0, void 0, void 0, 2, 50);
  ...
  function showMultiplePipelines(error) {
    ...
    if (listEl && details?.matches && Array.isArray(details.matches)) {
      listEl.innerHTML = details.matches.map((m) => `
  ...
  function renderSummaryCards(rollups, prevRollups = []) {
    ...
    if (prevRollups && prevRollups.length > 0) {
      ...
      renderDelta(elements["cycle-p50-delta"], calculatePercentChange(current.cycleP50, previous.cycleP50), true);
      renderDelta(elements["cycle-p90-delta"], calculatePercentChange(current.cycleP90, previous.cycleP90), true);
  ...
  function renderAIInsights(container, insights) {
    ...
    const icons = { critical: "\u{1F534}", warning: "\u{1F7E1}", info: "\u{1F535}" };
