...
const dataset_loader_1 = require("./dataset-loader");
const artifact_client_1 = require("./artifact-client");
 ...
 */
const IS_PRODUCTION = typeof window !== 'undefined' && window.process?.env?.NODE_ENV === 'production';
const DEBUG_ENABLED = !IS_PRODUCTION && ((typeof window !== 'undefined' && window.__DASHBOARD_DEBUG__) ||
...
function parseQueryParams() {
    ...
    if (datasetUrl) {
        ...
        if (!datasetUrl.startsWith('https://')) {
            return (0, error_types_1.createInvalidConfigError)('dataset', datasetUrl, 'Must be a valid HTTPS URL');
        }
        ...
        if (!IS_DEV) {
            try {
                ...
                if (!isAdoDomain) {
                    console.warn(`SECURITY: ?dataset= URL "${urlHost}" is not an Azure DevOps domain. ` +
                        `This parameter is intended for development only.`);
                }
            }
            catch (_e) {
                return (0, error_types_1.createInvalidConfigError)('dataset', datasetUrl, 'Invalid URL format');
            }
        }
        let warning = null;
        if (pipelineIdStr) {
    ...
    if (pipelineIdStr) {
        ...
        if (isNaN(pipelineId) || pipelineId <= 0) {
            return (0, error_types_1.createInvalidConfigError)('pipelineId', pipelineIdStr, 'Must be a positive integer');
        }
...
async function resolveConfiguration() {
    ...
    if (queryResult instanceof error_types_1.PrInsightsError) {
        throw queryResult;
    }
    // Mode: direct URL
    ...
    artifactClient = new artifact_client_1.ArtifactClient(targetProjectId);
...
async function resolveFromPipelineId(pipelineId, projectId) {
    ...
    const builds = await buildClient.getBuilds(projectId, [pipelineId], undefined, undefined, undefined, undefined, undefined, undefined, // reasonFilter
    2, // statusFilter: Completed
    6, // resultFilter: Succeeded (2) | PartiallySucceeded (4)
    undefined, undefined, 1 // top
    );
    if (!builds || builds.length === 0) {
        ...
        throw (0, error_types_1.createNoSuccessfulBuildsError)(name);
    }
    const latestBuild = builds[0];
    if (!latestBuild)
        throw new Error('Failed to retrieve latest build');
    // Check for aggregates artifact
    if (!artifactClient)
        throw new Error('ArtifactClient not initialized');
    const artifacts = await artifactClient.getArtifacts(latestBuild.id);
    const hasAggregates = artifacts.some(a => a.name === 'aggregates');
    if (!hasAggregates) {
        const definitions = await buildClient.getDefinitions(projectId, undefined, undefined, undefined, 2, undefined, undefined, undefined, [pipelineId]);
        const name = definitions?.[0]?.name || `ID ${pipelineId}`;
        throw (0, error_types_1.createArtifactsMissingError)(name, latestBuild.id);
    }
...
async function discoverAndResolve(projectId) {
    ...
    if (matches.length === 0) {
        throw (0, error_types_1.createSetupRequiredError)();
    }
...
async function discoverInsightsPipelines(projectId) {
    ...
    for (const def of definitions) {
        const builds = await buildClient.getBuilds(projectId, [def.id], undefined, undefined, undefined, undefined, undefined, undefined, 2, 6, undefined, undefined, 1);
        if (!builds || builds.length === 0)
            continue;
        const latestBuild = builds[0];
        if (!latestBuild)
...
async function init() {
    if (metricsCollector)
        metricsCollector.mark('dashboard-init');
    cacheElements();
    setupEventListeners();
    ...
    try {
        if (isLocalMode()) {
            ...
            loader = new dataset_loader_1.DatasetLoader(datasetPath);
            currentBuildId = null;
            const projectNameEl = document.getElementById('current-project-name');
            if (projectNameEl) {
        ...
        if (config.directUrl) {
            loader = new dataset_loader_1.DatasetLoader(config.directUrl);
            currentBuildId = null;
        }
...
async function refreshMetrics() {
    if (!currentDateRange.start || !currentDateRange.end || !loader)
        return;
    // Load current period data
    const rawRollups = await loader.getWeeklyRollups(currentDateRange.start, currentDateRange.end);
    const distributions = await loader.getDistributions(currentDateRange.start, currentDateRange.end);
...
async function updateFeatureTabs() {
    ...
    if (typeof loader.loadPredictions !== 'function')
        return;
...
function updateDatasetInfo(manifest) {
    ...
    if (runInfo) {
        ...
        if (runId)
            runInfo.textContent += ` | Run: ${runId.slice(0, 8)}`;
    }
}
...
function updateUrlState() {
    ...
    if (params.get('dataset'))
        newParams.set('dataset', params.get('dataset'));
...
function restoreStateFromUrl() {
    ...
    if (startParam && endParam) {
        ...
        if (elements['start-date'])
            elements['start-date'].value = startParam;
        if (elements['end-date'])
            elements['end-date'].value = endParam;
    }
