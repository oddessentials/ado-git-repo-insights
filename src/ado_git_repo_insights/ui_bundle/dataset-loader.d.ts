 ...
 */
export declare function normalizeRollups(rollups: any[] | null | undefined): Rollup[];
/**
...
export declare class DatasetLoader implements IDatasetLoader {
     ...
     */
    getWeeklyRollupsWithProgress(startDate: Date, endDate: Date, context: {
        org: string;
        project: string;
        repo: string;
        branch?: string;
        apiVersion?: string;
    }, onProgress?: (event: ProgressEvent) => void, cache?: RollupCache | null): Promise<RollupResult>;
    /**
     * Fetch a single week with semaphore control and bounded retry.
     */
    protected _fetchWeekWithRetry(weekStr: string, indexEntry: any, context: any, cache: RollupCache): Promise<{
        week: string;
        status: 'ok';
        data: Rollup;
    } | {
