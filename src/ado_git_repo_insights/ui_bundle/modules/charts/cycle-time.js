...
exports.renderCycleDistribution = renderCycleDistribution;
exports.renderCycleTimeTrend = renderCycleTimeTrend;
...
function renderCycleTimeTrend(container, rollups) {
    ...
    const svgContent = `
        ...
        const y = padding.top +
            chartHeight -
            (i / (yLabels.length - 1)) * chartHeight;
        return `<text class="line-chart-axis" x="${padding.left - 4}" y="${y + 3}" text-anchor="end">${(0, format_1.formatDuration)(val)}</text>`;
    })
        .join("")}
    ...
    (0, charts_1.addChartTooltips)(container, (dot) => {
        ...
        return `
            ...
            <div class="chart-tooltip-row">
                ...
                <span>${(0, format_1.formatDuration)(value)}</span>
            </div>
        `;
    });
}
