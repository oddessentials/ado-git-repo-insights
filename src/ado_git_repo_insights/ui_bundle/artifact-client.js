...
exports.MockArtifactClient = exports.AuthenticatedDatasetLoader = exports.ArtifactClient = void 0;
...
class ArtifactClient {
    ...
    async getArtifactFile(buildId, artifactName, filePath) {
        ...
        if (response.status === 401 || response.status === 403) {
            throw (0, error_types_1.createPermissionDeniedError)('read artifact files');
        }
    ...
    async getArtifactFileViaSdk(buildId, artifactName, filePath) {
        ...
        if (response.status === 401 || response.status === 403) {
            throw (0, error_types_1.createPermissionDeniedError)('read artifact file');
        }
        if (!response.ok) {
    ...
    async getArtifacts(buildId) {
        ...
        const response = await this._authenticatedFetch(url);
        if (response.status === 401 || response.status === 403) {
            throw (0, error_types_1.createPermissionDeniedError)('list build artifacts');
        }
        if (!response.ok) {
...
class AuthenticatedDatasetLoader {
    ...
    async loadManifest() {
        try {
            this.manifest = await this.artifactClient.getArtifactFileViaSdk(this.buildId, this.artifactName, 'dataset-manifest.json');
    ...
    validateManifest(manifest) {
        ...
        if (manifest.manifest_schema_version > SUPPORTED_MANIFEST_VERSION) {
            throw new Error(`Manifest version ${manifest.manifest_schema_version} not supported.`);
        }
        if (manifest.dataset_schema_version > SUPPORTED_DATASET_VERSION) {
            throw new Error(`Dataset version ${manifest.dataset_schema_version} not supported.`);
        }
        if (manifest.aggregates_schema_version > SUPPORTED_AGGREGATES_VERSION) {
            throw new Error(`Aggregates version ${manifest.aggregates_schema_version} not supported.`);
        }
    }
    async loadDimensions() {
        if (this.dimensions)
            return this.dimensions;
        this.dimensions = await this.artifactClient.getArtifactFileViaSdk(this.buildId, this.artifactName, 'aggregates/dimensions.json');
        return this.dimensions;
    }
    async getWeeklyRollups(startDate, endDate) {
        ...
        for (const weekStr of neededWeeks) {
            ...
            if (!indexEntry)
                continue;
            try {
                const rollup = await this.artifactClient.getArtifactFileViaSdk(this.buildId, this.artifactName, indexEntry.path);
    ...
    async getDistributions(startDate, endDate) {
        ...
        for (let year = startYear; year <= endYear; year++) {
            ...
            if (!indexEntry)
                continue;
            try {
                const dist = await this.artifactClient.getArtifactFileViaSdk(this.buildId, this.artifactName, indexEntry.path);
    ...
    async loadPredictions() {
        try {
            ...
            const data = await this.artifactClient.getArtifactFileViaSdk(this.buildId, this.artifactName, indexEntry.path);
