...
function createSetupRequiredError() {
    return new PrInsightsError(exports.ErrorTypes.SETUP_REQUIRED, 'Setup Required', 'No PR Insights pipeline found in this project.', {
        instructions: [
...
function createMultiplePipelinesError(matches) {
    return new PrInsightsError(exports.ErrorTypes.MULTIPLE_PIPELINES, 'Multiple Pipelines Found', `Found ${matches.length} pipelines with aggregates. Please specify which one to use.`, {
...
function createNoSuccessfulBuildsError(pipelineName) {
    return new PrInsightsError(exports.ErrorTypes.NO_SUCCESSFUL_BUILDS, 'No Successful Runs', `Pipeline "${pipelineName}" has no successful builds.`, {
        instructions: [
...
function createArtifactsMissingError(pipelineName, buildId) {
    return new PrInsightsError(exports.ErrorTypes.ARTIFACTS_MISSING, 'Aggregates Not Found', `Build #${buildId} of "${pipelineName}" does not have an aggregates artifact.`, {
        instructions: [
...
function createPermissionDeniedError(operation) {
    return new PrInsightsError(exports.ErrorTypes.PERMISSION_DENIED, 'Permission Denied', `You don't have permission to ${operation}.`, {
        instructions: [
...
function createInvalidConfigError(param, value, reason) {
    ...
    return new PrInsightsError(exports.ErrorTypes.INVALID_CONFIG, 'Invalid Configuration', `Invalid value for ${param}: "${value}"`, {
        reason,
        hint,
    });
}
