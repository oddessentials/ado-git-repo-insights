 ...
 *   npx ts-node scripts/stamp-extension-version.ts ${nextRelease.version}
 */
...
function parseVersion(version: string | undefined): ParsedVersion {
    if (!version) {
        ...
        console.error('Usage: npx ts-node stamp-extension-version.ts <version>');
        process.exit(1);
    }
 ...
 */
function validateVssSchema(vss: VssExtension): void {
 ...
 */
function validateTaskSchema(task: TaskJson): void {
 ...
 */
function readJson<T>(filePath: string, desc: string): T {
...
function main(): void {
    ...
    const vss = readJson<VssExtension>(PATHS.vss, 'vss-extension.json');
    ...
    const task = readJson<TaskJson>(PATHS.task, 'task.json');
