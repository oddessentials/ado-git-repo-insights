...
/**
 * Per-repo comments-density entry (Feature 335, INV-3-08).
 *
 * Inner-dict shape for one entry of the `rollup[W].by_repository_comments`
 * outer dict.  Atomic when the outer dict is present (INV-3-08): all four
 * fields MUST be present together with non-null typed values, or the
 * entire entry is a contract violation.  The outer dict's key is
 * `repository_id` directly — there is NO sentinel concept for the per-repo
...
export interface WeeklyRollup {
  ...
  // optional at the root level; when present every entry is atomic per
  // INV-3-08 (mirrors 333 INV-1-08 / 334 INV-2-08 at sub-object
...
/**
 * Validate the rollup-root `by_repository_comments` outer dict (Feature 335,
 * INV-3-08 atomicity).
 *
 * Atomicity posture mirrors Feature 334's `validateAuthorCommentsDensity`:
 * STRICT ERROR in BOTH strict and permissive modes.  The validator pushes
 * to `errors`, NOT `warnings` — INV-3-08 is a fresh contract introduced
 * by Feature 335 with no legacy emissions to grandfather, so renderers
 * may trust each entry's atomicity without per-field defensive null-checks.
 *
 * When `by_repository_comments` is present:
 *   - It MUST be an object (the outer Record<repositoryId, entry>).
 *   - It MUST NOT be the empty object `{}` (FR-1-10: the producer omits
 *     the key entirely when no buckets exist for the week).
 *   - Every entry value MUST satisfy the four-field atomicity contract
 *     (`thread_count` / `comment_count` / `active_thread_count` /
 *     `coverage_partial` all present, numeric fields non-null integers,
 *     boolean coverage_partial, INV-3-07 ordering).
 *
 * No sentinel concept — `repository_id` is FK-protected at `models.py:88`
 * (CL-03 / FR-1-03 / INV-3-12), so every key is a raw `repository_id`
 * string.  There is no reserved-literal handling.
 *
 * Capability-off (FR-3-03 + INV-3-09) is signalled by the entire
 * `by_repository_comments` key being absent — that path simply skips this
 * validator with no warning.  Empty outer dict (`{}`) under capability-on
 * is a contract violation (FR-1-10).
 */
function validateRepositoryCommentsDensity(
  ...
  for (const [key, entry] of entries) {
    ...
    // INV-3-07 ordering: active_thread_count <= thread_count per entry.
    const entryThread = entry.thread_count;
    const entryActive = entry.active_thread_count;
    if (
      isNumber(entryThread) &&
...
export function validateRollup(
  ...
  // Feature 335 per-repo comments-density (rollup root).  Optional outer
  // dict; per-entry atomic per INV-3-08.  STRICT ERROR atomicity posture
  // mirrors Feature 334 / 333 (no legacy emissions to grandfather).
  // Capability-off (FR-3-03 + INV-3-09) is signalled by the entire
  // `by_repository_comments` key being absent — that path skips the
  // validator with no warning.  Empty `{}` under capability-on is a
  // contract violation per FR-1-10 (caught inside the validator).
  if (
    Object.prototype.hasOwnProperty.call(data, "by_repository_comments") &&
    data.by_repository_comments !== undefined
  ) {
