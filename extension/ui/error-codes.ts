...
export function createErrorMessage(
    ...
    const error = (ErrorCodes as Record<string, ErrorCodeDefinition>)[errorKey] ?? ErrorCodes.UNKNOWN;
    return {
        code: error.code,
...
if (typeof window !== 'undefined') {
    (window as Window & { ErrorCodes?: typeof ErrorCodes }).ErrorCodes = ErrorCodes;
    (window as Window & { getErrorByCode?: typeof getErrorByCode }).getErrorByCode = getErrorByCode;
    (window as Window & { createErrorMessage?: typeof createErrorMessage }).createErrorMessage = createErrorMessage;
}
