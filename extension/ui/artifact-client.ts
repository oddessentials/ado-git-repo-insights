...
export class ArtifactClient {
    ...
    async initialize(): Promise<ArtifactClient> {
        ...
        this.authToken = typeof tokenResult === 'string' ? tokenResult : (tokenResult as any).token;
     ...
     */
    async getArtifactFile(buildId: number, artifactName: string, filePath: string): Promise<any> {
     ...
     */
    async hasArtifactFile(buildId: number, artifactName: string, filePath: string): Promise<boolean> {
        this._ensureInitialized();
     ...
     */
    async getArtifactMetadata(buildId: number, artifactName: string): Promise<any | null> {
        this._ensureInitialized();
     ...
     */
    async getArtifactFileViaSdk(buildId: number, artifactName: string, filePath: string): Promise<any> {
        this._ensureInitialized();
     ...
     */
    private _buildFileUrl(buildId: number, artifactName: string, filePath: string): string {
        const normalizedPath = filePath.startsWith('/') ? filePath : '/' + filePath;
     ...
     */
    protected async _authenticatedFetch(url: string, options: RequestInit = {}): Promise<Response> {
...
export class AuthenticatedDatasetLoader implements IDatasetLoader {
    ...
    async getWeeklyRollups(startDate: Date, endDate: Date): Promise<any[]> {
        ...
        for (const weekStr of neededWeeks) {
            ...
            const indexEntry = (this.manifest as any).aggregate_index?.weekly_rollups?.find(
    ...
    async getDistributions(startDate: Date, endDate: Date): Promise<any[]> {
        ...
        for (let year = startYear; year <= endYear; year++) {
            ...
            const indexEntry = (this.manifest as any).aggregate_index?.distributions?.find(
