...
export interface RollupCache {
    ...
    ttlMs: number;
    clock: () => number;
    makeKey(params: { week: string; org: string; project: string; repo: string; branch?: string; apiVersion?: string }): string;
    get(key: string): any;
...
export function createRollupCache(clock: () => number = Date.now): RollupCache {
    ...
    const entries = new Map<string, { value: any; createdAt: number; touchedAt: number }>();
...
export class DatasetLoader implements IDatasetLoader {
    ...
    async getWeeklyRollupsWithProgress(
        startDate: Date,
        endDate: Date,
        context: { org: string; project: string; repo: string; branch?: string; apiVersion?: string },
        ...
        const useCache: RollupCache = cache || ({
    ...
    protected async _fetchWeekWithRetry(
        ...
        context: any,
        cache: RollupCache
    ): Promise<{ week: string; status: 'ok'; data: Rollup } | { week: string; status: 'auth' | 'missing' | 'failed'; error?: any }> {
        let retries = 0;
     ...
     */
    protected validatePredictionsSchema(predictions: any): { valid: boolean; error?: string } {
     ...
     */
    protected validateInsightsSchema(insights: any): { valid: boolean; error?: string } {
