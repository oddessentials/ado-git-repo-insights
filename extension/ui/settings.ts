...
async function init(): Promise<void> {
    try {
        ...
        const projectInput = document.getElementById('project-id') as HTMLInputElement | null;
...
async function tryLoadProjectDropdown(): Promise<void> {
    const dropdown = document.getElementById('project-select') as HTMLSelectElement;
    ...
    try {
        ...
        if (projects && projects.length > 0) {
            ...
            for (const project of projects.sort((a: any, b: any) => a.name.localeCompare(b.name))) {
    ...
    return new Promise((resolve, reject) => {
        VSS.require(['TFS/Core/RestClient'], (CoreRestClient: any) => {
            try {
                ...
                client.getProjects().then((projects: any[]) => {
                    resolve(projects || []);
                }).catch((error: any) => {
                    reject(error);
                });
...
async function loadSettings(): Promise<void> {
    ...
    try {
        ...
        // Set pipeline ID
        const pipelineInput = document.getElementById('pipeline-id') as HTMLInputElement;
...
async function saveSettings(): Promise<void> {
    ...
    const pipelineInput = document.getElementById('pipeline-id') as HTMLInputElement | null;
 ...
 */
async function validatePipeline(pipelineId: number, projectId: string): Promise<{ valid: boolean; name?: string; buildId?: number; error?: string }> {
            ...
            try {
                ...
                client.getDefinitions(
                    ...
                    client.getBuilds(
                        ...
                        resolve({ valid: false, error: `Build check failed: ${e.message}` });
                    });
                }).catch((e: any) => {
                    resolve({ valid: false, error: `Definition fetch failed: ${e.message}` });
                });
            } catch (e: any) {
 ...
 */
async function discoverPipelines(): Promise<Array<{ id: number; name: string; buildId: number }>> {
    return new Promise((resolve) => {
        VSS.require(['TFS/Build/RestClient'], (BuildRestClient: any) => {
            try {
                const client = BuildRestClient.getClient();
                ...
                client.getDefinitions(projectId, null, null, null, 2, 50).then(async (definitions: any[]) => {
    ...
    try {
        ...
        for (const match of matches) {
            document.getElementById(`select-pipeline-${match.id}`)?.addEventListener('click', () => {
                ...
                showStatus(`Pipeline ${match.id} selected - click Save to confirm`, 'info');
            });
        }
...
function setupEventListeners(): void {
    ...
    // Enter key saves
    document.getElementById('pipeline-id')?.addEventListener('keypress', (e: KeyboardEvent) => {
        if (e.key === 'Enter') {
            saveSettings();
        }
    });
    document.getElementById('project-id')?.addEventListener('keypress', (e: KeyboardEvent) => {
        if (e.key === 'Enter') {
            saveSettings();
        }
    });
