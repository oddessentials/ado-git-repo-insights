...
describe('DatasetLoader', () => {
    ...
    describe('validatePredictionsSchema', () => {
        it('returns { valid: true } for valid input', () => {
            ...
            const result = (loader as any).validatePredictionsSchema(validPredictions);
        ...
        it('returns { valid: false, error } for missing schema_version', () => {
            ...
            const result = (loader as any).validatePredictionsSchema(invalidPredictions as any);
            expect(result.valid).toBe(false);
        ...
        it('returns { valid: false, error } for unsupported schema version', () => {
            ...
            const result = (loader as any).validatePredictionsSchema(futurePredictions as any);
            expect(result.valid).toBe(false);
    ...
    describe('validateInsightsSchema', () => {
        ...
        it('returns { valid: false, error } for missing schema_version', () => {
            ...
            const result = (loader as any).validateInsightsSchema(invalidInsights as any);
            expect(result.valid).toBe(false);
            expect(result.error).toContain('schema_version');
        });
    ...
    describe('loadPredictions state machine', () => {
        ...
        it('returns { state: "disabled" } when feature flag is false', async () => {
            (loader as any).manifest = { ...validManifest, features: { predictions: false } };
    ...
    describe('loadInsights state machine', () => {
        ...
        it('returns { state: "disabled" } when feature flag is false', async () => {
            (loader as any).manifest = { ...validManifest, features: { ai_insights: false } };
