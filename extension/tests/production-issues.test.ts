...
describe("Issue 1: PartiallySucceeded Builds Acceptance", () => {
  ...
  describe("Build filtering logic", () => {
     ...
     */
    const filterBuilds = (
      builds: { id: number; result: number; name: string }[],
      resultFilter: number,
    ) => {
...
describe("Issue 2: Stale Settings and Auto-Discovery Fallback", () => {
  ...
  describe("Configuration resolution with fallback", () => {
    ...
    const createResolveConfiguration = (options: {
      queryPipelineId?: number | null;
      savedPipelineId?: number | null;
      resolveFromPipelineIdFn: (id: number) => Promise<{ buildId: number }>;
      discoverAndResolveFn: () => Promise<{ buildId: number }>;
      clearStalePipelineSettingFn: () => Promise<void>;
    }) => {
      const {
        queryPipelineId,
        savedPipelineId,
  ...
  describe("Re-discover pipelines functionality", () => {
    ...
    const createDiscoverPipelines = (mockClient: {
      getDefinitions: () => Promise<{ id: number; name: string }[]>;
      getBuilds: (id: number) => Promise<{ id: number }[] | null>;
      getArtifacts: (id: number) => Promise<{ name: string }[]>;
    }) => {
      return async function discoverPipelines() {
        ...
        for (const def of definitions) {
          ...
          if (!artifacts.some((a: { name: string }) => a.name === "aggregates"))
            continue;
  ...
  describe("End-to-end scenario: Extension reinstall", () => {
    ...
    it("next load uses discovery directly after stale setting cleared", async () => {
      const savedPipelineId = { value: null as number | null }; // Already cleared from previous load
      ...
      const resolveConfiguration = async () => {
        if (savedPipelineId.value) {
          return await mockResolveFromPipelineId(savedPipelineId.value);
        }
        return await mockDiscoverAndResolve();
      };
...
describe("Regression Prevention", () => {
  ...
  describe("Fallback behavior must be preserved", () => {
    ...
    it("query param errors should NOT trigger discovery fallback", async () => {
      const discoveryTriggered = { value: false };
      ...
      expect(discoveryTriggered.value).toBe(false);
    });
  });
...
describe("Phase 5 Feature Flag (Predictions & AI Insights)", () => {
  ...
  describe("initializePhase5Features function", () => {
    ...
    const createInitializePhase5Features = (featureFlag: boolean) => {
      return function initializePhase5Features(
        mockTabs: { show: () => void }[],
      ) {
        if (featureFlag) {
