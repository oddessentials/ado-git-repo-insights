...
const originalLocalDashboardMode = (window as { LOCAL_DASHBOARD_MODE?: boolean }).LOCAL_DASHBOARD_MODE;
...
describe("SDK Module", () => {
    ...
    afterAll(() => {
        ...
        if (originalLocalDashboardMode !== undefined) {
            (window as { LOCAL_DASHBOARD_MODE?: boolean }).LOCAL_DASHBOARD_MODE = originalLocalDashboardMode;
        }
    ...
    describe("isLocalMode", () => {
        ...
        it("returns false when LOCAL_DASHBOARD_MODE is false", () => {
            (window as { LOCAL_DASHBOARD_MODE?: boolean }).LOCAL_DASHBOARD_MODE = false;
            expect(isLocalMode()).toBe(false);
        });

        it("returns true when LOCAL_DASHBOARD_MODE is true", () => {
            (window as { LOCAL_DASHBOARD_MODE?: boolean }).LOCAL_DASHBOARD_MODE = true;
            expect(isLocalMode()).toBe(true);
        });

        it("returns false for truthy non-boolean values", () => {
            // Test that only explicit true triggers local mode
            (window as unknown as { LOCAL_DASHBOARD_MODE: number }).LOCAL_DASHBOARD_MODE = 1;
            expect(isLocalMode()).toBe(false);
        });
    });
    ...
    describe("getLocalDatasetPath", () => {
        ...
        it("handles paths with special characters", () => {
            (window as { DATASET_PATH?: string }).DATASET_PATH = "/path/with spaces/and%20encoding";
    ...
    describe("getBuildClient", () => {
        it("resolves with Build REST client via VSS.require", async () => {
            ...
            const mockVSS = {
                require: jest.fn((modules: string[], callback: (...args: unknown[]) => void) => {
    ...
    describe("getExtensionDataService", () => {
        it("resolves via VSS.getService with ExtensionData service ID", async () => {
            ...
            expect(mockVSS.getService).toHaveBeenCalledWith("extension-data-service-id");
