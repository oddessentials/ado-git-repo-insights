...
describe('Local Mode Integration', () => {
     ...
     */
    function simulateLocalModeInit(options = {}) {
        const {
            datasetPath = './dataset',
            manifestExists = true
        } = options;
        ...
        it('shows error state when dataset-manifest.json missing', async () => {
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    errorTitle.textContent = 'Dataset Not Found';
                    errorMessage.textContent = 'Ensure the analytics pipeline has run successfully.';
                }
            } catch (e) {
                // Network error path
            }
