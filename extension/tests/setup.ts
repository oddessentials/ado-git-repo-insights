...
declare global {
    var mockFetchResponse: (data: any, options?: { status?: number; ok?: boolean }) => Promise<any>;
...
let performanceMeasures: Array<{ name: string; duration: number; entryType: string }> = [];
...
if (!(global.performance as any).measure) {
    (global.performance as any).measure = (name: string, startMark: string, endMark: string) => {
...
if (!(global.performance as any).getEntriesByName) {
    (global.performance as any).getEntriesByName = (name: string, type: string) => {
        if (type === 'measure') {
...
global.mockFetchResponse = (data: any, options: { status?: number; ok?: boolean } = {}) => {
