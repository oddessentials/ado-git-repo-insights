...
describe('Sprint 1: Trend Deltas & Metrics', () => {
    ...
    const createCalculatePercentChange = () => {
        return function calculatePercentChange(current: number | null | undefined, previous: number | null | undefined) {
    ...
    const createRenderDelta = () => {
        return function renderDelta(element: HTMLElement | null, percentChange: number | null, inverse = false) {
            if (!element) return;
...
describe('Sprint 3: Sparklines & Moving Average', () => {
    ...
    const createRenderSparkline = () => {
        return function renderSparkline(element: HTMLElement | null, values: number[] | null | undefined) {
...
describe('Sprint 4: Charts & Tooltips', () => {
    ...
    const createAddChartTooltips = () => {
        return function addChartTooltips(container: HTMLElement, contentFn: (dot: Element) => string) {
...
describe('Sprint 5: Comparison Mode & Export', () => {
    ...
    describe('downloadRawDataZip', () => {
         ...
         */
        const createMockArtifactClient = (options: { artifact?: any; fetchOk?: boolean; fetchStatus?: number; statusText?: string } = {}) => {
            return {
...
describe('Client-Side Filtering (applyFiltersToRollups)', () => {
    ...
    const createApplyFiltersToRollups = () => {
        return function applyFiltersToRollups(rollups: any[], filters: { repos: string[], teams: string[] }) {
...
describe('Sprint 2: Filter Management', () => {
    ...
    const createFilterChipFn = () => {
        return function createFilterChip(type: string, value: string, label: string) {
    ...
    describe('Filter UI Updates', () => {
        ...
        it('clears all filters on clear button click', () => {
            ...
            (repoFilter.querySelector('option[value="backend"]') as HTMLOptionElement).selected = true;
            (teamFilter.querySelector('option[value="platform"]') as HTMLOptionElement).selected = true;
            ...
            expect((repoFilter.querySelector('option[value=""]') as HTMLOptionElement).selected).toBe(true);
            expect((repoFilter.querySelector('option[value="backend"]') as HTMLOptionElement).selected).toBe(false);
        });
    });
    ...
    describe('restoreFiltersFromUrl', () => {
        ...
        it('restores repo filters from URL params', () => {
            ...
            const repoFilter = document.getElementById('repo-filter');
            expect((repoFilter?.querySelector('option[value="backend"]') as HTMLOptionElement)?.selected).toBe(true);
            expect((repoFilter?.querySelector('option[value="frontend"]') as HTMLOptionElement)?.selected).toBe(true);
        });
    ...
    describe('populateFilterDropdowns', () => {
        ...
        it('populates repo dropdown from dimensions', () => {
            ...
            expect((repoFilter?.querySelector('option[value="repo1"]') as HTMLOptionElement)?.textContent).toBe('Backend API');
            expect((repoFilter?.querySelector('option[value="repo2"]') as HTMLOptionElement)?.textContent).toBe('Frontend App');
        });
        ...
        it('populates team dropdown from dimensions', () => {
            ...
            expect((teamFilter?.querySelector('option[value="team1"]') as HTMLOptionElement)?.textContent).toBe('Platform Team');
        });
