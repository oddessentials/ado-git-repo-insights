...
const syntheticFixtureTest = pythonSubprocessSupport.supported ? test : test.skip;
...
describe("Synthetic Fixture Consumer Validation", () => {
  ...
  syntheticFixtureTest("1000 PR fixture passes loadManifest validation", async () => {
  ...
  syntheticFixtureTest("generated manifest has correct schema versions", async () => {
    const fixturePath = generateFixture(1000, 42);
  ...
  syntheticFixtureTest("deterministic generation: same seed produces identical manifest structure", async () => {
  ...
  describe("cross-dimensional round-trip (T016)", () => {
    syntheticFixtureIt("Python-generated by_team_and_repo survives load and produces exact filter results", async () => {
    ...
    syntheticFixtureIt("cross-dim consistency: sum of team-repo pr_counts equals team total", () => {
      const outputDir = generateFixture(100, 42);
      const manifestPath = path.join(outputDir, "dataset-manifest.json");
      const manifest = JSON.parse(fs.readFileSync(manifestPath, "utf-8"));
    ...
    syntheticFixtureIt("filter with non-existent team/repo returns zero pr_count", async () => {
      const outputDir = generateFixture(100, 42);
      const manifestPath = path.join(outputDir, "dataset-manifest.json");
      const manifest = JSON.parse(fs.readFileSync(manifestPath, "utf-8"));
