...
const { DatasetLoader, fetchSemaphore, createRollupCache } = require('../ui/dataset-loader');
...
describe('Phase 4: Chunked Loading', () => {
    ...
    describe('createRollupCache with injected clock', () => {
        it('returns cached value within TTL', () => {
            ...
            const key = cache.makeKey({ week: '2026-W01', org: 'test', project: 'proj', repo: 'repo' });
        ...
        it('evicts oldest by touchedAt when at capacity', () => {
            ...
            const week2Key = cache.makeKey({ week: '2026-W02', org: 'test', project: 'proj', repo: 'repo' });
            cache.get(week2Key); // Updates touchedAt

            // Add week 53 - should evict week 1 (oldest touchedAt)
            const week53Key = cache.makeKey({ week: '2026-W53', org: 'test', project: 'proj', repo: 'repo' });
            cache.set(week53Key, { week: 53 });

            const week1Key = cache.makeKey({ week: '2026-W01', org: 'test', project: 'proj', repo: 'repo' });
        ...
        it('throws if cache key missing required fields', () => {
            ...
            expect(() => {
                cache.makeKey({ week: '2026-W01', org: 'test' }); // Missing project, repo
    ...
    describe('getWeeklyRollupsWithProgress', () => {
        it('returns explicit missingWeeks[] for 404s', async () => {
            global.fetch = jest.fn((url) => {
                ...
                return mockFetchResponse({ week: weekMatch ? weekMatch[1] : '2026-W01', pr_count: 10 });
            });
        ...
        it('returns explicit failedWeeks[] for 5xx after retry', async () => {
            ...
            const w03Calls = callCount; // Should be at least 2 for W03's retries
            expect(w03Calls).toBeGreaterThanOrEqual(2);
        ...
        it('throws AUTH_REQUIRED if authError && data.length === 0', async () => {
            global.fetch = jest.fn(() => mockFetch401());
        ...
        it('returns degraded state if partial auth success', async () => {
            global.fetch = jest.fn((url) => {
                if (url.includes('2026-W01')) return mockFetchResponse({ week: '2026-W01', pr_count: 10 });
                if (url.includes('2026-W02')) return mockFetchResponse({ week: '2026-W02', pr_count: 15 });
                return mockFetch403();
            });
        ...
        it('retries go through semaphore', async () => {
            ...
            global.fetch = jest.fn((url) => {
                ...
                return mockFetchResponse({ week: url.match(/W\d+/)[0], pr_count: 10 });
            });

            const semaphoreAcquires = [];
            const originalAcquire = fetchSemaphore.acquire.bind(fetchSemaphore);
            fetchSemaphore.acquire = jest.fn(() => {
                semaphoreAcquires.push(Date.now());
            ...
            expect(fetchSemaphore.acquire).toHaveBeenCalled();
            expect(semaphoreAcquires.length).toBeGreaterThanOrEqual(2); // Initial + retries
        });
        ...
        it('stress test: randomized completion order with fixed seed', async () => {
            ...
            global.fetch = jest.fn((url) => {
                ...
                return new Promise(resolve => {
                    setTimeout(() => {
                        const weekMatch = url.match(/(2026-W\d+)/);
                        resolve(mockFetchResponse({ week: weekMatch ? weekMatch[1] : '2026-W01', pr_count: weekNum * 5 }));
                    }, delay);
                });
            });
