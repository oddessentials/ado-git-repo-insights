...
import { DatasetLoader, fetchSemaphore, createRollupCache } from '../ui/dataset-loader';
    ...
    describe('getWeeklyRollupsWithProgress', () => {
        ...
        it('retries go through semaphore', async () => {
            ...
            (global as any).fetch = jest.fn((url: string) => {
                ...
                return mockFetchResponse({ week: match ? match[0] : 'W01', pr_count: 10 });
            });

            const originalAcquire = fetchSemaphore.acquire.bind(fetchSemaphore);
            const acquireSpy = jest.spyOn(fetchSemaphore, 'acquire').mockImplementation(() => {
